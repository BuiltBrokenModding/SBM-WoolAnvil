...
public class BlockGenericAnvil extends BlockFalling {
  ...
  public BlockGenericAnvil(AnvilProperties properties, EnumVariants variant) {
    ...
    setLightOpacity(0);
    setCreativeTab(WoolAnvils.creativeTab);
    setHardness(1.0F);
  ...
  public static IBlockState damage(IBlockState state) {
    ...
    switch (block.variant) {
      case NORMAL:
        return WoolAnvils.anvilDamageMap.get(block).getDefaultState().withProperty(FACING, enumfacing);
      case CHIPPED:
        return WoolAnvils.anvilDamageMap.get(block).getDefaultState().withProperty(FACING, enumfacing);
      case DAMAGED:
      default:

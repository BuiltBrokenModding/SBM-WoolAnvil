...
public class ContainerGenericAnvil extends Container
{
    ...
    public ContainerGenericAnvil(InventoryPlayer playerInventory, World worldIn, EntityPlayer player, BlockAnvil anvil)
    {
        this(playerInventory, worldIn, BlockPos.ORIGIN, player, anvil);
    }

    public ContainerGenericAnvil(InventoryPlayer playerInventory, final World worldIn, final BlockPos blockPosIn, EntityPlayer player, BlockAnvil genericAnvil)
    {
        ...
        this.maximumCap = genericAnvil.cap;
        this.enchantability = Math.max(genericAnvil.enchantability, 0.001);
        this.durMultiplier = genericAnvil.durabilityMultiplier;
        ...
        this.addSlotToContainer(new Slot(this.outputSlot, 2, 134, 47)
        {
            ...
            public ItemStack onTake(EntityPlayer playerIn, @Nonnull ItemStack stack)
            {
                ...
                //damage the anvil
                if (!playerIn.capabilities.isCreativeMode && (!worldIn.isRemote && iblockstate.getBlock() instanceof BlockAnvil && playerIn.getRNG().nextFloat() < breakChance || breakChance == -1))
                {

                    BlockAnvil.damage(iblockstate);
                }
            ...
            {
                if (genericAnvil.enchantability == -1)
                {
                    return -1;
                }
    ...
    public static boolean isLeather(ItemStack stack)
    {
        if (stack.getItem() instanceof ItemArmor)
        {
            return ((ItemArmor) stack.getItem()).getArmorMaterial().equals(ItemArmor.ArmorMaterial.LEATHER);
        }
        else
        {
    ...
    public void updateRepairOutput()
    {
        ...
        else
        {
            ...
            {
                if (!onAnvilChange(this, itemstack, itemstack2, outputSlot, repairedItemName, j))
                {
                    return;
                }
                    ...
                    for (Enchantment enchantment1 : map1.keySet())
                    {
                        if (enchantment1 != null)
                        {
                            ...
                            for (Enchantment enchantment : map.keySet())
                            {
                                ...
                                {//func_191560_c checks if ench can apply with ench1 and vice versa
                                    flag1 = false;
                                    ++i;
                                }
                            }
    ...
    {
        return this.theWorld.getBlockState(this.selfPosition).getBlock() instanceof BlockAnvil && playerIn.getDistanceSq((double) this.selfPosition.getX() + 0.5D, (double) this.selfPosition.getY() + 0.5D, (double) this.selfPosition.getZ() + 0.5D) <= 64.0D;
    }
